...
use windows::{
    ...
    Win32::System::Com::{CoInitializeEx, CoUninitialize, CoCreateInstance, CoSetProxyBlanket},
};
...
#[cfg(windows)]
fn load_proc(module: &str, name: &str) -> usize {
    unsafe {
        let module = format!("{module}\0");
        let name = format!("{name}\0");
        let handle = GetModuleHandleA(PCSTR(module.as_ptr())).expect("GetModuleHandleA");
        let proc = GetProcAddress(handle, PCSTR(name.as_ptr())).expect("GetProcAddress");
        proc as usize
    }
}
...
fn com_api() -> &'static ComApi {
    COM.get_or_init(|| unsafe {
        ComApi {
            co_init: mem::transmute(load_proc("ole32.dll", "CoInitializeEx")),
            co_uninit: mem::transmute(load_proc("ole32.dll", "CoUninitialize")),
            co_create: mem::transmute(load_proc("ole32.dll", "CoCreateInstance")),
            co_blanket: mem::transmute(load_proc("ole32.dll", "CoSetProxyBlanket")),
            sys_alloc: mem::transmute(load_proc("oleaut32.dll", "SysAllocStringLen")),
            sys_free: mem::transmute(load_proc("oleaut32.dll", "SysFreeString")),
            sys_len: mem::transmute(load_proc("oleaut32.dll", "SysStringLen")),
        }
    })
}
...
#[cfg(not(windows))]
fn com_api() -> &'static ComApi {
    static COM_DUMMY: OnceLock<ComApi> = OnceLock::new();
    COM_DUMMY.get_or_init(|| ComApi {
        co_init: dummy_co_init,
        co_uninit: dummy_co_uninit,
        co_create: dummy_co_create,
        co_blanket: dummy_co_blanket,
        sys_alloc: dummy_sys_alloc,
        sys_free: dummy_sys_free,
        sys_len: dummy_sys_len,
    })
}

#[cfg(not(windows))]
unsafe extern "system" fn dummy_co_init(_: *const c_void, _: u32) -> i32 { 0 }
#[cfg(not(windows))]
unsafe extern "system" fn dummy_co_uninit() { }
#[cfg(not(windows))]
unsafe extern "system" fn dummy_co_create(_: *const GUID, _: *const c_void, _: u32, _: *const GUID, _: *mut *mut c_void) -> i32 { 0 }
#[cfg(not(windows))]
unsafe extern "system" fn dummy_co_blanket(_: *mut c_void, _: u32, _: u32, _: *const u16, _: u32, _: u32, _: *const c_void, _: u32) -> i32 { 0 }
#[cfg(not(windows))]
unsafe extern "system" fn dummy_sys_alloc(_: *const i8, _: u32) -> *mut u16 { std::ptr::null_mut() }
#[cfg(not(windows))]
unsafe extern "system" fn dummy_sys_free(_: *mut u16) { }
#[cfg(not(windows))]
unsafe extern "system" fn dummy_sys_len(_: *const u16) -> u32 { 0 }
