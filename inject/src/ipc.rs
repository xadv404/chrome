...
const ERROR_NO_DATA: u32 = 0x0000_00E9;

/// Server-side named pipe used to receive the payload master key.
...
impl PipeServer {
    /// Creates a byte-mode named pipe server.
    ...
    pub unsafe fn accept(&self) -> Result<(), ()> {
        match ConnectNamedPipe(self.handle, None) {
            Ok(()) => Ok(()),
            Err(error) if error.code().0 as u32 == ERROR_PIPE_CONNECTED => Ok(()),
            Err(_) => Err(()),
        }
    }

    /// Reads JSON lines until a master key or error message is received.
    pub unsafe fn wait_for_key(&mut self, timeout: Duration) -> Result<Vec<u8>, String> {
        let deadline = Instant::now() + timeout;
        loop {
            if Instant::now() >= deadline {
                return Err("timed out waiting for payload response".into());
            }
            if let Some(line) = self
                .try_read_line()
                .map_err(|_| "failed to read from named pipe".to_string())?
            {
    ...
    unsafe fn try_read_line(&mut self) -> Result<Option<String>, ()> {
        ...
        let mut read = 0u32;
        match ReadFile(self.handle, Some(&mut chunk), Some(&mut read), None) {
            Ok(()) => {}
            Err(error) if error.code().0 as u32 == ERROR_NO_DATA => return Ok(None),
            Err(_) => return Err(()),
        }
        if read == 0 {
            return Ok(None);
...
fn parse_line(line: &str) -> Option<Vec<u8>> {
    ...
    let hex = value.get("master_key_hex")?.as_str()?;
